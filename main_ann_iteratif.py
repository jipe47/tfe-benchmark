...
#nbr_iter = 10
range_hiddenSize = range(5, 15)
nbr_iter = 5

#array_correct_seasonality = [True, False]
array_correct_seasonality = [False]
#array_epochs = [1, 15]
array_epochs = [500]
...
nbr_loop = len(rows) * len(array_epochs) * len(array_correct_seasonality)

for row in rows:
	
	for epochs in array_epochs:
		for correct_seasonality in array_correct_seasonality:
				...
				for hiddenSize in range_hiddenSize:
					...
					for j in range(nbr_iter):
						if it % display_step == 0:
							print "ANN - curve " , currentCurve , " / " , nbr_loop , " (id_curve = " , row["id_curve"] , ", id_zone = " , row["id"] , ") - " , it , " / " , total_test
